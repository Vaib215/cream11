...
const getTotalCredits = (team: Player[]) =>
  team.reduce(
    (sum, p) => sum + (typeof p.credits === "number" ? p.credits : 0),
    0
  );

const countByTeam = (team: Player[]) =>
  team.reduce<Record<string, number>>((acc, p) => {
...
const ensureNumericCredits = (p: Player): Player => ({
  ...p,
  credits:
    typeof p.credits === "number" && !Number.isNaN(p.credits) ? p.credits : 8,
});
...
const buildCheapestTeamWithRoleCounts = (
  allPlayersIn: Player[],
...
const adjustDreamTeamForBudget = (
  dreamTeamIn: Player[],
  allPlayersIn: Player[],
  creditLimit = 100
): Player[] => {
  ...
  let iter = 0;
  while (
    getTotalCredits(dreamTeam) > creditLimit &&
    iter < safeGuardMaxIterations
  ) {
    iter++;
    // Sort current players by credits desc (try replacing most expensive first)
    const sortedByCost = [...dreamTeam].sort(
      (a, b) => (b.credits || 0) - (a.credits || 0)
    );
    let replaced = false;
    ...
    for (const current of sortedByCost) {
      ...
      const candidates = allPlayers
        .filter(
          (p) => p.role === current.role && playerKey(p) !== playerKey(current)
        )
        .filter((p) => (p.credits || 0) < (current.credits || 0))
        .filter((p) => !dreamTeam.some((t) => playerKey(t) === playerKey(p)))
        .sort((a, b) => (a.credits || 0) - (b.credits || 0));

      const currentWithout = dreamTeam.filter(
        (p) => playerKey(p) !== playerKey(current)
      );
  ...
  dreamTeam = dreamTeam.map((p) => ({
    ...p,
    credits: Math.round(((p.credits || 0) + Number.EPSILON) * 10) / 10,
  }));
...
export const fetchUpcomingMatches = async (): Promise<{ matches: Match[] }> => {
  try {
    const prompt = `
            Based on your knowledge, list cricket matches happening around TODAY (Indian time: ${new Date().toLocaleDateString(
              "en-US",
              { timeZone: "Asia/Kolkata" }
            )}). If you know of matches today, return those.
            Otherwise, list the top 5 most anticipated upcoming international or major league (e.g., IPL, BBL, T20 World Cup) cricket matches.
            For each match, provide the two teams playing, the tournament name, the date, and the venue.
    ...
    const text = await generateContentWithFallback(prompt, { jsonMode: true });
...
export const generateDreamTeam = async (
  match: Match
  ...
  try {
    const prompt = `
            ...
            **CREAM11 SCORING SYSTEM TO CONSIDER:**
            ...
            REASONING LOGIC:
            Based on your knowledge of cricket and these teams, provide a comprehensive analysis explaining:
               - **Team Composition Validation**: Confirm the team meets all Cream11 rules (player counts, credit budget, team distribution)
               - **Captain & Vice-Captain Selection**: Justify choices based on known form, consistency, and point-scoring potential
               - **Statistical Analysis**: Use your knowledge of recent performance stats
               - **Form Analysis**: Consider typical performance patterns
            ...
            1. 'reasoning': Your comprehensive analysis as a string
            2. 'dreamTeam': Array of EXACTLY 11 players with: name, team, role ('Batsman', 'Bowler', 'All-Rounder', 'Wicket-Keeper'), credits (7.0-10.5), isCaptain (boolean), isViceCaptain (boolean)
            3. 'allPlayers': Complete squad lists from both teams with name, team, role, and estimated credits
            ...
            Based on your knowledge of cricket, player statistics, and team squads, generate the best possible fantasy team.
            Respond with ONLY a valid JSON object containing 'reasoning', 'dreamTeam' and 'allPlayers' keys.
            YOUR TEXT RESPONSE SHOULD ONLY BE A JSON AND NOTHING ELSE.
        `;

    const text = await generateContentWithFallback(prompt, { jsonMode: true });
    const result: DreamTeamResponse = parseJsonWithFixes(cleanJsonString(text));

    // Ensure reasoning field exists
    if (!result.reasoning) {
      result.reasoning =
        "Team generated based on player knowledge and fantasy cricket best practices.";
    }

    // --- Post-processing: enforce budget locally ---
    if (result?.dreamTeam?.length && result?.allPlayers?.length) {
      const adjustedTeam = adjustDreamTeamForBudget(
        result.dreamTeam as Player[],
        result.allPlayers as Player[],
        100
      );
