...
const WetPaintButton = ({children, className}: {className?: string; children: React.ReactNode}) => {
  return (
    <button className={cn("group relative rounded bg-red-500 px-4 py-2.5 font-semibold text-white transition-colors hover:bg-red-600", className)}>
      {children}
...
const Drip: React.FC<DripProps> = ({ left, height, delay }) => {
  return (
    ...
    >
      ...
      <div
        style={{ height }}
        className="w-2 rounded-b-full bg-red-500 transition-colors group-hover:bg-red-600"
      />
      ...
      >
        <g clipPath="url(#clip0_1077_28)">
          <path
            ...
            className="fill-red-500 transition-colors group-hover:fill-red-600"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
      ...
      <motion.div
        ...
        className="absolute top-full h-2 w-2 rounded-full bg-red-500 transition-colors group-hover:bg-red-600"
      />
    </motion.div>
  );
};
